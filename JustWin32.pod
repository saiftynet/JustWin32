...
Created when text detected.  Widgets have ids in the form `stattext<id>`, and can be accessed programmatically using `$frame->{stattext<id>}`. The content can be changed using `$frame->setLabel(<id>,<newContent>)` e.g. 
